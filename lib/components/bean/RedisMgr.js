...
exports.removeCacheByValue = function(tableName, foreignValue, toRem, cb) {
    ...
    if (util.isArray(toRem)) {
        ...
        exports.redisClient.pipeline(pipeArr).exec(cb);
    } else {
        exports.redisClient.lrem(key, toRem, cb);
    }
};

exports.removeCacheByKey = function(tableName, foreignValue, cb) {
    var key = tableName + ":" + foreignValue;
    exports.redisClient.del(key, cb);
};
 ...
 * @param cb
 */
exports.deleteRedisCacheByFather = function(tableName, primaryValue, foreignValue, cb) {
    ...
    var getSonOrder = function(tName, primaryValue, _cb) {
        ...
        sonNames.forEach((sonN)=>{
            ...
            sonPipe.push(["lrange", sonN + ":" + primaryValue, 0, -1]); // 寻找孙数据
        ...
        exports.redisClient.pipeline(sonPipe).exec((err, data)=>{
            ...
            data.forEach((aData, idx)=>{
                ...
                aData.forEach((aSonData)=>{
                    ...
                    // 将孙数据作为父数据,向下继续查找子数据
                    var theData = JSON.parse(aSonData[0]);
                    getSonOrder(sonN, theData[aSonT[PRIMARY_KEY]], checkEnd);
                });
            });
    ...
    getSonOrder(tableName, primaryValue, ()=>{
        exports.redisClient.pipeline(pipeArr).exec(cb); // 统一删除待删除数据
    });
};
