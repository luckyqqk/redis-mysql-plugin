 ...
 * @param redisIndex
 */
exports.updateData = function(tableName, jsonValue, redisIndex) {
    RedisMgr.updateRedisCache(tableName, jsonValue, redisIndex, (err)=>{
        if (!!err) logger.error(`update cache failed::${err}`);
        else {
            // save db
            ...
            DBMgr.query(sql, [], function (err, data) {
                if (!!err) logger.error(`updateData failed::${err}`);
                //RedisMgr.addRedisCache(tableName, jsonValue, (err)=>{
                //    if (!!err) logger.error(`addRedisCache failed::${err}`);
                //});
            });
        }
